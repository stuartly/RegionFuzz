...
namespace {
    ...
    class EntryDegree : public ModulePass {

    public:

        static char ID;

        EntryDegree() : ModulePass(ID) {}
...
char EntryDegree::ID = 0;

std::string EntryDegree::getBBName(BasicBlock &BB) {
...
bool EntryDegree::runOnModule(Module &M) {

    std::string outFile = "";
    std::map<std::string, double> BB_TO_FairReachDegree;
    ...
    for (auto &F: M) {
        ...
        for (auto &B:F) {
            ...
            BB_TO_FairReachDegree.insert(pair<std::string, double>(bb_name, FairReachDegree));
        }
    }


    //output to file
    std::ofstream bb_to_fairReachDegree;
    if (outDir.empty()) {
        FATAL("setting -outDir\n");
    } else {
        outFile = outDir + "/BBEntryDegree.txt";
    }

    bb_to_fairReachDegree.open(outFile, std::ofstream::out | std::ofstream::app);

    for (auto &ele: BB_TO_FairReachDegree) {
        bb_to_fairReachDegree << ele.first << "," << ele.second << "\n";
    }

    bb_to_fairReachDegree.close();
